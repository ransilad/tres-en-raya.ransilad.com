...
The system SHALL support starting a new board with the same online players after a win or draw when both players agree.

#### Scenario: Both players request rematch
- **WHEN** both online players choose to play again after a completed match
- **THEN** the system resets the online board, clears the result, and starts a new online match with player X to move first
